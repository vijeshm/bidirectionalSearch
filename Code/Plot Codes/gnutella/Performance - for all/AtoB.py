...
def simple_query(GLearnt):
	...
	for A in NodeList:
		for B in NodeList[NodeList.index(A):]:
			...
			TwoWayRandomFullPath = TwoWayRandomWalk(G,src,dstn)
			TwoWayAdamicFullPath = TwoWayAdamicWalk(G,src,dstn)
			ShortestPath = nx.shortest_path(G, src, dstn)

			TwoWayRandomFullPaths.append(float(len(TwoWayRandomFullPath))/len(ShortestPath))
			TwoWayAdamicFullPaths.append(float(len(TwoWayAdamicFullPath))/len(ShortestPath))
	...
	print "\n"
	print "Avg of TwoWayRandomFullPaths :" , numpy.average(TwoWayRandomFullPaths)
	print "Avg of TwoWayAdamicFullPaths :" , numpy.average(TwoWayAdamicFullPaths)
	return [numpy.average(TwoWayRandomFullPaths), numpy.average(TwoWayAdamicFullPaths)]

def comparison_query(GLearnt, trials):
	i = 1
	...
	return (1/(RandomWalk_OneWay_time*numpy.average(RandomWalk_OneWay_Paths)),1/(RandomWalk_TwoWay_time*numpy.average(RandomWalk_TwoWay_Paths)), 1/(PlainAdamic_OneWay_time*numpy.average(PlainAdamic_OneWay_Paths)), 1/(PlainAdamic_TwoWay_time*numpy.average(PlainAdamic_TwoWay_Paths)))
